...
\begin{equation}
    \ket{\alpha_j} = \prod_{i\sigma} \left(c^\dag_{i\sigma}\right)^n \ket{0},
    \quad
    n\in\{0,1\},
\end{equation}
which allows us to write arbitrary wave functions as linear combinations
\begin{equation}
...
we can split our Hamiltonian into blocks of fixed $N_\uparrow$ and $N_\downarrow$
...
In the first step,
the one particle basis $\{\ket{\phi_i}\}$ was optimized to the so-called
...
which decomposes the wave function
\begin{equation}
...
In $\ket{\psi_\mathrm{II}}$ which contains the remaining chain sites we neglect almost all states.
In the simplest approximation (labeled $p=0$)
we enforce that all valences sites are fully filled (denoted $\ket{\mathbb{1}_v}$) while
conduction sites are completely empty (denoted $\ket{\mathbb{0}_c}$).
The wave function can thus be written as
\begin{equation}
    \ket{\psi_\mathrm{II}} = \ket{\mathbb{1}_v} \otimes \ket{\mathbb{0}_c}.
\end{equation}

For $p=1$ we allow additionally one excitation
which creates an electron or hole excitation.
...
In this work the highest excitation we allow is $p=2$.
...
Thus, we can broaden our delta-peaks from TODO
