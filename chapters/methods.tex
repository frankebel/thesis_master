...
which is represented as an $n\times n$ matrix\footnote{
    The matrix dimension $n$ is not the same as
    the discretization to $n$ one-particle wave functions in the previous section.
    In fact, the former is much larger than the latter.
}.
Therefore,
...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
still guaranteeing orthogonality against previous states $Q_i^\dag Q_j = \mathbb{0}_{p\times p}$
...
Here, the subscript $K_{11}$ is meant as the first $p\times p$ block of the matrix $K$
\begin{equation}
    C^\pm_\omega
    =
    B_0 \left[\frac{1}{\omega + \mi0^+ \mp T}\right]_{11} B_0.
...
while the block version returns $p^2$ correlators simultaneously.
...
\end{subequations}
where $M$ is the Krylov size and $p$ the size of each block.
...
Given our representation in \zcref{eq:self-energy-discrete},
...
\end{flalign}
with $\Sigma^{(-2)}$ equaling the negative second moment.
