...
Here, the subscript $K_{11}$ is meant as the first $q\times q$ block of the matrix $K$
\begin{equation}
    C^\pm_\omega
    =
    B_0 \left[\frac{1}{\omega + \mi0^+ \mp T}\right]_{11} B_0.
...
and thus $C_\omega$ is also hermitian\footnote{
    ...
    significant digits. This is avoided by setting matrices to $K \to 1/2 (K + K^\dag)$.
}.
...
Thus, in the metallic region we have no pole located at zero\footnote{
    We will discuss this statement for our discrete model in the results section.
    % TODO: add reference
    \fe{TODO\@: add reference}
}.
