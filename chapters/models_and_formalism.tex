...
In this work we look at the half-filled system $\epsilon - \mu = U/2$
...
\begin{equation}
    \Delta_\omega = \sum_{k\sigma} \frac{|V_k|^2}{\omega + \mi0^+ - \epsilon_k}.
\end{equation}
...
the first four moments of the Green's function are\footnote{
    The general case is available in~\cite{Potthoff1997}.
}
\begin{align}
    G^{(0)} & = 1,                             \\
    G^{(1)} & = 0,                             \\
...
The analytic moments for half-filling are\footnote{
    Again, the general case is available in~\cite{Potthoff1997}.
}
\begin{align}
    \Sigma^\mH   & = \frac{U}{2},   \\
    \Sigma^{(0)} & = \frac{U^4}{4}, \\
...
After adding the interacting term of \zcref{eq:H-Anderson-interaction}, we can solve the impurity
