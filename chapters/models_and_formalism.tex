...
In solid-states physics we study crystals in which
atoms are periodically arranged on a Bravais lattice.
...
It is an infinite graph where each vertex is connected to $z$ neighbors\footnote{
    ...
    in contrast to the Bravais lattice which is constructed by tiling cells characterized
    by basis vectors.
}.
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        Density of states for different connectivities $z$ with hopping set to unity $t^*=1$
        (adapted from~\cite{Wallerberger2016}).
    }
\end{figure}
It is connected to three nodes (marked in red) which in turn are also connected
to three nodes (each red node is connected to the original black node and two blue nodes).
...
the total probability of movement is $zp^*$.
In the limit of infinite connectivity $z\to\infty$ (infinite dimensions) this will diverge,
necessitating a renormalization.
Thus, the rescaled probability is set to $p = p^*/z$ such that
the total probability $zp$ stays finite.
...
For the Bethe lattice, the density of states\footnote{
    This distribution is also known as the Wigner semicircle distribution
    which is the eigenvalue distribution of random matrices~\cite{Wigner1955, Mehta2004}.
    They are related to the DOS in as much as a random walk on the Bethe lattice.
    As the graph has no closed loops,
    a particle must necessarily take the same path back to return to its origin~\cite{Wallerberger2025}.
}
(DOS) for $z\to\infty$ reads~\cite{Eckstein2005}
\begin{equation}
    \rho(\omega) = \frac{1}{2\muppi {t^*}^2} \sqrt{4{t^*}^2 - \omega^2}
\end{equation}
which is shown in \zcref{subfig:bethe-density}.
We can define the bandwidth $W=4t^*$ as the difference between the highest and lowest frequency
where this expression vanishes.
This relation can be rewritten by using the half-bandwidth $D = W/2$ instead:
\begin{equation}
...
Hubbard model directly\footnote{
    We only obtain quantities in the DMFT approximation which is exact for infinite dimensions.
}~\cite{Hubbard1997},
we nevertheless want to mention it briefly due to its importance in DMFT\@.
Denoting the electron creation operator of site $i$ and spin $\sigma\in\{\uparrow,\downarrow\}$
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        The hopping amplitude between neighboring sites is given by $t$
...
\end{align}
which has the same structure as \zcref{eq:H-Hubbard} but only for a single site.
...
\begin{equation}
    ...
    \label{eq:H-Anderson-impurity-bath}
\end{equation}
...
\begin{figure}[ht]
    ...
    \caption{
        ...
        where a full site corresponds to one double occupation.
...
\begin{equation}
    \Delta(\omega) = \sum_{k} \frac{|V_k|^2}{\omega + \mi0^+ - \epsilon_k}.
\end{equation}
This function describes the dynamics of the bath, i.e.,
how electrons of a given energy can hop in and out of the impurity.
It is introduced here,
as it will be important in DMFT (\zcref{sec:dmft}).

\section{Green's function and correlators}%
\label{sec:greens-function-correlators}

The retarded Green's function is defined as~\cite{Economou2006}
\begin{equation}
...
In the Heisenberg picture\footnote{
    ...
    based on the time-dependent Schrödinger equation.
}
the time evolution of the annihilation operator is given as
$d_{i\sigma}(t) = \me^{\mi H t} d_{i\sigma} \me^{-\mi H t}$.
At zero temperature $T=0$, the expectation value is evaluated with respect to the ground state
...
e.g., at time $0$ a particle is added at site $i$ with spin $\sigma$ to the ground state.
The system then evolves as dictated by the Hamiltonian.
At a later time $t$ the particle is removed from the same site and
the resulting state is then projected onto the initial state.
The local Green's function (for equal sites $i$) can therefore be interpreted as
the probability amplitude of a particle returning to its origin at time $t$~\cite{Economou2006}.
...
we will denote the Fourier transform\footnote{
    ...
    and allows us to approach it
    from above the real axis $\omega + \mi 0^+$ (positive times $\Theta(t)$)
    which is called the retarded Green's function.
    If one comes from below the real axis $\omega - \mi 0^+$
    (negative times $\Theta(-t)$),
    it is called the advanced Green's function.
    Using different approaches for $\omega\gtrless0$ gives rise to the so-called
    causal and anti-causal correlator.
}
of \zcref{eq:correlator-time} as
\begin{equation}
    C_\omega
    \coloneq
    \lim_{\delta\to0^+}
...
we will write function arguments or variables as subscripts, e.g.,
$C(\omega) \to C_\omega$ to keep notation compact.
As \zcref{eq:correlator-time} contains two terms (from the anticommutator)
and the Fourier transform is linear,
$C_\omega = C^+_\omega + C^-_\omega$ can be combined from
partial correlators with poles on the positive and negative real axis,
respectively.
...
Given such a full decomposition of $H$ allows us to write \zcref{eq:correlator-frequency}
in the so-called \emph{Lehmann representation}
\begin{subequations}
    \begin{align}
        C^+_\omega
         & =
        \sum_n
        \frac{\sandwich{\psi_0}{A}{n}\sandwich{n}{B}{\psi_0}}{\omega + \mi0^+ - E_n}, \\
        C^-_\omega
         & =
        \sum_n
        \frac{\sandwich{\psi_0}{B}{n}\sandwich{n}{A}{\psi_0}}{\omega + \mi0^+ - E_n}.
    \end{align}%
    \label{eq:lehmann}% chktex 24
\end{subequations}
We define \emph{diagonal correlators} as those where $A=B^\dag$, e.g.,
our local Green's function.
For these,
the numerators of \zcref{eq:lehmann} simplify to positive real values
\begin{subequations}
    \begin{align}
        \sandwich{\psi_0}{A}{n}\sandwich{n}{B}{\psi_0}
         & =
        |\sandwich{n}{A^\dag}{\psi_0}|^2, \\
        \sandwich{\psi_0}{B}{n}\sandwich{n}{A}{\psi_0}
         & =
        |\sandwich{n}{A}{\psi_0}|^2.
    \end{align}%
\end{subequations}
...
\begin{figure}[ht]
    ...
    \caption{
        Real and imaginary part of the non-interacting Green's function of the
        Bethe lattice in infinite dimensions in units of the half-bandwidth $D$.
    }%
...
\end{equation}
which can be generalized to the \emph{spectral part} of $C$
\begin{equation}
...
but related by Kramers-Kronig transformations (Hilbert transform)~\cite{Economou2006}
\begin{subequations}
    \begin{align}
        \Re C_\omega
         & =
    ...
    \end{align}%
\end{subequations}
with $\mathcal{P}$ denoting the Cauchy principal value integral.
...
\end{equation}
which is shown in \zcref{fig:bethe-greens-function}.
...
its hybridization function is just a rescaled Green's function~\cite{Georges1996}
\begin{equation}
    \Delta_\omega = \frac{D^2}{4} G_\omega.
    \label{eq:hybridization-function-bethe}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:moments}
\end{equation}
allowing us to write $C_\omega$ in a high-frequency expansion~\cite{Potthoff1997}
\begin{equation}
...
which allows us to formulate the moments as~\cite{Potthoff1997}
\begin{equation}
...
\begin{subequations}
    \begin{align}
        G^{(0)} & = 1,                             \\
        G^{(1)} & = 0,                             \\
        G^{(2)} & = \frac{U^2}{4} + \frac{D^2}{4}, \\
        G^{(3)} & = 0.
    \end{align}%
    \label{eq:spectrum-moments}%
\end{subequations}
...
is the so-called \emph{self-energy} defined by the Dyson equation\footnote{
    This relation can be motivated by Feynman diagrams.
}~\cite{Economou2006}
\begin{equation}
...
to write down a high-energy expansion~\cite{Potthoff1997}
\begin{equation}
...
\begin{subequations}
    \begin{align}
        ...
        \Sigma^{(1)} & = 0,
    \end{align}%
    \label{eq:self-energy-moments}%
\end{subequations}
with every odd moment vanishing.

Although the Dyson equation is exact,
...
To mitigate this,
\citeauthor{Bulla1998}~\cite{Bulla1998} calculated the self-energy using a quotient of two correlators.
In this work we go a step further by using the symmetric improved estimator~\cite{Kaufmann2019}
introduced by \citeauthor{Kugler2022}~\cite{Kugler2022}
which will be summarized in the following.
We introduce the auxiliary operator
\begin{equation}
    q_\sigma = \comm{d_\sigma}{H_\mathrm{int}} = U d_\sigma n_{-\sigma}
\end{equation}
with the interacting Hamiltonian from \zcref{eq:H-Anderson-interaction}.
Here, $-\sigma$ indicates opposite spin.
This allows us to calculate the Hartree term directly
\begin{equation}
    \Sigma^\mH = \expval{\acomm{q^\dag_\sigma}{d^{\pdag}_\sigma}}.
\end{equation}
We then employ a shift\footnote{
    As noted by \citeauthor{Kugler2022},
    this is especially convenient for particle-hole symmetric systems.
    Strictly speaking this shift is not necessary,
    but it will simplify our calculation as it will create a diagonal matrix
    in \zcref{eq:amplitudes-half-filling}.
}
\begin{equation}
...
A given Hubbard Hamiltonian is solved\footnote{
    Strictly speaking we do not need to solve the Hamiltonian but find
    the corresponding interacting Green's function.
}
by a self-consistency loop,
which repeatedly maps between the Hubbard model and Anderson impurity model until
their Green's function coincide.
Let us briefly recapitulate this DMFT self-consistency loop,
for further details see~\cite{Georges1996, Held2007}:
...
\end{equation}
by averaging over the Brillouin zone $\mathrm{BZ}$.
In DMFT, this is approximated by assuming that the self-energy is purely local
($\Sigma_{\vec{k}\omega} \to \Sigma_\omega$)
\begin{equation}
    G_{\mloc,\omega}
    \approx
    \frac{1}{V_\mBZ} \int_\mBZ \md\vec{k}\> \frac{1}{\omega + \mi0^+ + \mu - \epsilon_{\vec{k}} - \Sigma_\omega}.
    \label{eq:greens-function-local-approx}
\end{equation}
We then enforce that this Green's function coincides with the impurity Green's function
of the corresponding Anderson impurity model: $G_{\mloc,\omega} \equiv \mathcal{G}_\omega$.

Rearranging the Dyson equation (\zcref{eq:Dyson}) for the impurity
then gives us the non-interacting impurity Green's function of this Anderson impurity model
\begin{align}
    (\mathcal{G}_{0,\omega})^{-1}
     & =
...
Given these terms, we can write down a non-interacting Hamiltonian
of \zcref{eq:H-Anderson} which leads to the given Green's function $\mathcal{G}_{0,\omega}$.
After adding the interacting term of \zcref{eq:H-Anderson-interaction},
we can solve the impurity Hamiltonian
yielding us a new impurity Green's function $\mathcal{G}_\omega$
and self-energy $\Sigma_\omega$.
Subsequently,
we use this impurity self-energy to
calculate a new lattice Green's function using \zcref{eq:greens-function-local-approx},
closing our self-consistency loop.
These steps are repeated until the calculated impurity Green's function equals the local
lattice Green's function $\mathcal{G}_\omega \equiv G_{\mloc,\omega}$.
...
\begin{equation}
    \Delta(\omega) = \Delta_{0}(\omega + \mu - \mu_0 - \Sigma_\omega),
    \label{eq:hybridization-update}
\end{equation}
