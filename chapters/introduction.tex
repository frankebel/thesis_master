...
the interplay between electrons, driven by the Coulomb interaction
...
which maps the system to that of an impurity coupled to a non-interacting bath.
...
multi-orbital impurities which is often necessary to describe realistic materials.
However, the problem is that sampling is done on the imaginary axis which requires
an ill-conditioned inversion~\cite{Jarrell1996, Beach2000, Kaufmann2023}
to obtain real frequency data.
In contrast, NRG --- which is designed for impurity problems ---
gives quantities on the real axis,
has excellent agreement for small frequencies
but suffers at larger ones due to logarithmic discretization~\cite{Bulla2008}.
ED methods can be applied very broadly, obtain quantities on the real axis too but
suffer from discretization artifacts.
...
In this work we combine their approach
together with the symmetric improved estimator~\cite{Kugler2022} introduced for NRG\@.
...
We explain the natural impurity orbitals~\cite{Lu2014, Lu2019} and RAS allowing us to
solve our system in polynomial instead of exponential time.
...
Only here we introduce some broadening to plot smooth spectra.
