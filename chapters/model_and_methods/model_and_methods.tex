...
\begin{equation}
    | \psi \rangle = \sum_{|\phi\rangle \in \mathcal{S}} \alpha_{|\phi\rangle} | \phi \rangle
\end{equation}
using any orthonormal set $\mathcal{S}$ of basis functions.
With the occupation number,
we can express the number of electrons in the system as $n_\uparrow + n_\downarrow$ with
\begin{equation}
    n_\sigma = n_{i,\sigma} + \sum_{k=1}^N n_{b,k\sigma}.
\end{equation}
If the number of electrons is fixed for $N$ bath sites and the impurity,
the set of Slater determinants can be written as
\begin{equation}
    \mathcal{S} =
    \left\{
    |n_{i,\uparrow},n_{b,1\uparrow},\ldots,n_{b,N\uparrow}\rangle
    \otimes
    |n_{i,\downarrow},n_{b,1\downarrow},\ldots,n_{b,N\downarrow}\rangle:
    n_{i,\sigma}, n_{b,k\sigma} \in \{0, 1\}
    \right\}.
\end{equation}
To fully describe a state $|\psi\rangle$, a combination of
\begin{equation}
    M = \binom{N+1}{n_\uparrow} \cdot \binom{N+1}{n_\downarrow}
\end{equation}
determinants are necessary, which for a half-filled system ($n_\uparrow = n_\downarrow = (N+1)/2$)
with $N+1=140$ sites is approximately $\num{9e81}$,
...
Potential hopping between sites is again shown by lines.
...
The values $b_0$, $B_0$ normalize the initial state(s) for the (block) Lanczos routine.
