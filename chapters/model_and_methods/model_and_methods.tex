...
\fe{NOTE: I do not want to intruduce Feynman diagrams and irreducibility as
...
In the limit $d\to\infty$ it can be solved by
dynamical mean-field theory (DMFT) \cite{Georges1996}
which takes one site of the lattice as a representative
and averages all spacial fluctuations keeping only time correlations
(hence the term \emph{dynamical mean-field}).

A given Hamiltonian is solved by a self-consistency loop which
maps between the Hubbard model and Anderson impurity model.
Given a Hubbard model, we can define its local Green's function
\begin{equation}
    G_{\mloc,\omega}
    =
...
of the Anderson impurity model: $G_{\mloc,\omega} \equiv G_\omega$.
...
\end{align}
which is then rewritten as a term containing the impurity energy $\epsilon - \mu$
and hybridization function
\begin{equation}
    \Delta_\omega = \sum_{k\sigma} \frac{\abs{V_{k\sigma}}^2}{\omega - \epsilon_{k\sigma}}.
\end{equation}
Given these terms, we can create the non-interacting Hamiltonian \zcref{eq:anderson-non-interacting}.
After adding the interacting term of \zcref{eq:anderson-interaction}, we can solve the impurity
Hamiltonian using our solver which gives us a new impurity Green's function $G_\omega$
and self-energy $\Sigma_\omega$.
We can then use this impurity self-energy to
calculate a new lattice Green's function using \zcref{eq:greens-function-local}
closing our self-consistency equations.

For the Bethe lattice (\zcref{sec:bethe-lattice}) one can skip the lattice Green's function
and directly update the hybridization \cite{Lu2014}:
\begin{equation}
    \Delta_\omega = \Delta_{0}(\omega + \mu - \mu_0 - \Sigma_\omega),
\end{equation}
with derivation given in TODO. % TODO: derive DMFT for Bethe lattice
Due to the self-similarity in the Bethe lattice,
the hybridization function is just a rescaled Green's function
\begin{equation}
    \Delta_\omega = \frac{D^2}{4} G_\omega,
\end{equation}
notably $\Delta_{0,\omega}$ being a rescaled semicircular DOS \zcref{eq:dos-semicircle}.
