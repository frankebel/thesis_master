...
In this thesis we developed a DMFT impurity solver which works on the real axis at zero temperature.
By utilizing a smart choice of basis (natural impurity orbitals)
and restricted active space (RAS) approach,
we successfully transform our problem from an exponentially scaling one $\bigO{4^N}$
to a polynomial one $\bigO{N^p}$ with $p$ a tradeoff between speed and accuracy.
This allows us to couple the impurity to hundreds of bath sites,
which was previously the limiting factor in exact diagonalization (ED).
...
without taking a difference quotient.
...
It may be possible to improve this calculation by using no broadening instead.

There are multiple directions to improve and enhance the current code base:
First,
it was not possible to find a metastable insulting solution.
...
If one applies the Hamiltonian onto a wave function, hopping elements
will mix Slater determinants such that two different ones end up in the same state,
...
and \texttt{DMFT.jl} (\url{https://github.com/frankebel/DMFT.jl}).
